...
/**
 * Packed baseline content for the 9 study sections + their static chrome
 * (eyebrow / title / desc). Extracted into its own module so
 * `useStudySections` (fallback source) and `app/study/page.tsx` (renderer) can
 * both import it WITHOUT a circular dependency (page ↔ hook).
 *
 * content_json shapes (parent design.md §1, verified against
 * `backend/src/main/resources/study/baseline/*.json`):
 *  - study-route:     `{ lead, steps: [{label, desc}] }`        (object)
 *  - training-plan:   `{ weeks: [{week,title,focus,tasks[]}],
 *                       review: {title, paragraphs[]} }`        (object, nested)
 *  - the other 7:     arrays of section-specific item objects.
 *
 * The two object-wrapped constants here are reshaped from child-1's flat
 * arrays to match the backend seed exactly (so API === baseline byte-for-byte
 * and the renderer is shape-agnostic). Field names drop the old `icon` field
 * (no icon library — design.md §2).
 */

// ----------------------------------------------------------------------------
// Item-shape interfaces (also consumed by SectionView for safe rendering).
// ----------------------------------------------------------------------------

export interface StudyRouteStep {
  label: string;
  desc: string;
}
...
// ----------------------------------------------------------------------------
// Content constants — semantics ported from child-1 page.tsx, reshaped to match
// backend seed JSON for study-route / training-plan.
// ----------------------------------------------------------------------------

const STUDY_ROUTE: StudyRouteContent = {
  lead: '九个模块按顺序练：从全卷扫描到作文三步法，每一步都对应考试中的具体动作。',
  steps: [
...
const MATERIAL_MOVES: KnowledgeCard[] = [
  {
    title: '先分层再摘点',
    eyebrow: '材料处理法',
    summary:
...
const TRAINING_PLAN: TrainingPlanContent = {
  weeks: [
    {
      week: '第 1 周',
      title: '审题和分层',
...
// ----------------------------------------------------------------------------
// Aggregate baseline — the API-offline / empty fallback source.
// ----------------------------------------------------------------------------

export const BASELINE_SECTIONS: Record<SectionKey, unknown> = {
  'study-route': STUDY_ROUTE,
  'exam-scan': EXAM_SCAN,
  'review-rules': REVIEW_RULES,
  'material-moves': MATERIAL_MOVES,
  'question-guides': QUESTION_GUIDES,
  'format-matrix': FORMAT_MATRIX,
  'essay-rules': ESSAY_RULES,
  pitfalls: PITFALLS,
  'training-plan': TRAINING_PLAN,
};

// ----------------------------------------------------------------------------
// Section chrome + ordering — the static SectionHead metadata (NOT part of
// content_json; eyebrow/title/desc are page-level). study-route's desc is
// data-driven off `content.lead` at render time (baseline lead === the static
// desc below, so byte-identical for the fallback path).
// ----------------------------------------------------------------------------
...
export interface SectionMeta {
  ...
  eyebrow: string;
  title: string;
  /** Static desc; used for every section except study-route (data-driven). */
  desc: string;
}
...
/** Human label per section_key (used in history / review UI). */
export const SECTION_LABELS: Record<SectionKey, string> = {
  'study-route': '学习路径',
  'exam-scan': '考场作答地图',
  'review-rules': '审题三件事',
  'material-moves': '材料处理法',
  'question-guides': '题型知识库',
  'format-matrix': '应用文格式矩阵',
  'essay-rules': '大作文三步法',
  pitfalls: '常见误区自检',
  'training-plan': '训练路径',
};
