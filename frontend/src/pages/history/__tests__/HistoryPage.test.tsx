...
const installAuthenticatedSession = () => {
  saveStoredAuthToken({
...
describe('HistoryPage live refresh subscription', () => {
  const buildAuthedFetchMock = () => {
    let historyListCalls = 0;
    const fetchMock = vi.fn((input: RequestInfo | URL) => {
      const url = String(input);
      if (url.endsWith('/api/v1/auth/me')) {
        return jsonResponse(authedUser);
      }
      if (url.endsWith('/api/v1/writings/history?limit=50')) {
        historyListCalls += 1;
        return jsonResponse({ items: [rawWritingItem, legacyWritingItem] });
    ...
    return {
      fetchMock,
      getHistoryListCalls: () => historyListCalls,
    };
  };

  it('reloads the list when a refresh signal for the same user arrives', async () => {
    installAuthenticatedSession();
    const { getHistoryListCalls } = buildAuthedFetchMock();

    renderAuthenticatedPage();
    await screen.findByText(displayId(rawWritingItem.id));
    await waitFor(() => {
      expect(getHistoryListCalls()).toBe(1);
    });
  ...
  it('ignores refresh signals scoped to a different user', async () => {
    installAuthenticatedSession();
    const { getHistoryListCalls } = buildAuthedFetchMock();
  ...
  it('preserves search and filter state across an automatic reload', async () => {
    const user = userEvent.setup();
    installAuthenticatedSession();
    const { getHistoryListCalls } = buildAuthedFetchMock();
  ...
  it('coalesces overlapping refresh signals into at most one follow-up reload', async () => {
    installAuthenticatedSession();
    const { getHistoryListCalls } = buildAuthedFetchMock();
    ...
    await waitFor(() => {
      expect(getHistoryListCalls()).toBeGreaterThanOrEqual(2);
    });
    await new Promise(resolve => setTimeout(resolve, 50));
    expect(getHistoryListCalls()).toBeLessThanOrEqual(3);
  });
});
