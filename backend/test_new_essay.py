...
from app.schemas.essay import EssayGradingRequest
from app.services.ai_service import AIService
import json

def test_new_essay_api():
    """测试新的申论批改API"""
    
    # 测试数据
    test_request = EssayGradingRequest(
...
三者协同发挥作用，构筑了黄河的立体科技防线：以实体原型黄河为基础，通过数字孪生黄河进行全流域、系统性的实时动态模拟和智慧化管理；同时，利用"模型黄河"对关键、具体问题开展精准的物理模型试验，其试验成果既能为实体黄河治理提供科学依据，也能验证、优化数字孪生模型。三者虚实结合、相互支撑，共同推动了黄河治理的系统化和精准化。
    ...
    try:
        ...
        result = ai_service.grade_essay(test_request)
        
        print("✅ 新API测试成功！")
        print(f"评分: {result.score}")
        print(f"结构反馈: {result.structure_feedback}")
        print(f"内容反馈: {result.content_feedback}")
        print(f"语言反馈: {result.language_feedback}")
